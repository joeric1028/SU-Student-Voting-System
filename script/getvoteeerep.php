    ...
    <body>
        <table class="w3-table-all w3-hoverable">
            ...
            <?php
                ...
                $q = intval($_GET['q']);
                $result = mysqli_query($con,"SELECT * FROM user INNER JOIN course ON course_idcourse = idcourse INNER JOIN college ON college_idcollege = idcollege WHERE candidatetype ='Electrical Engineering Representative' AND idcollege ='$q' AND candidateyear ='$currentyear';");
                if($result != FALSE)
                {
                    if(mysqli_num_rows($result) != 0)
                    {
                        ...
                        {
                            $result2 = mysqli_query($con, "SELECT * FROM user INNER JOIN course ON course_idcourse = idcourse INNER JOIN college ON college_idcollege = idcollege WHERE college.name ='{$row['name']}' AND course.name = 'Bachelor of Science in Electrical Engineering';");
